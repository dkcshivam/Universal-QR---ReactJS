...
function CreateMultipleProduct() {
  ...
  const handleProductImagesChange = (index, event) => {
    ...
    const updatedRows = [...productRows];
    updatedRows[index].productImages = [...updatedRows[index].productImages, ...files];
  ...
  return (
    <div
      className="fixed left-0 right-0 w-screen min-h-screen bg-white shadow-md overflow-x-auto p-4 box-border"
    >
      <div className="flex items-center justify-between mb-5 px-2">
        <h1 className="text-xl font-semibold">Create Multiple Products</h1>
        <button className="save-all-btn flex items-center gap-2" onClick={handleSaveAll}>
          <FaSave />
          Save All Products
        </button>
      </div>
      <table className="create-product-table" style={{ width: '100%', tableLayout: 'fixed' }}>
        <colgroup>
              ...
              <td style={{ width: '90px', textAlign: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'center' }}>
                  ...
                  >
                    ...
                    />
                    <FaPlus style={{ color: '#2563eb', fontSize: 18, margin: 0 }} />
                  </label>
                </div>
              </td>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', justifyContent: 'center' }}>
                  {row.productImageUrls &&
                    row.productImageUrls.map((url, imgIdx) => (
                      <div key={imgIdx} style={{ position: 'relative', width: 32, height: 32 }}>
                        <img
                          src={url}
                          alt="Product"
                          style={{ width: 32, height: 32, objectFit: 'cover', borderRadius: 6, border: '1px solid #eee' }}
                        />
                        <button
                          type="button"
                  ...
                  >
                    ...
                    />
                    <FaPlus style={{ color: '#2563eb', fontSize: 16, margin: 0 }} />
                  </label>
                </div>
              </td>
