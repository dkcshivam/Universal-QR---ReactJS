...
const AddProduct = () => {
  ...
  async function handlesubmit() {
    ...
    const res = await axios.post("http://shivam-mac.local:8000/api/v1.0/qr/products/create/", {
      "name": productName,
      "cover_image": coverImage ? coverImage.file : null,
      "product_images": productImages.map((img) => img.file),
      "quantity": quantity,
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4 space-y-10">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-6 space-y-6">
        ...
        <div>
          ...
          {!coverImage ? (
            <label className="border-2 border-dashed border-indigo-300 rounded-md bg-gray-50 flex flex-col items-center justify-center h-52 text-center px-4 py-6 text-gray-500 cursor-pointer transition">
              ...
              <p className="mt-1 font-medium text-gray-600">Upload cover image</p>
            </label>
          ) : (
        ...
        <div>
          ...
          {productImages.length === 0 ? (
            <label className="border-2 border-dashed border-indigo-300 rounded-md bg-gray-50 flex flex-col items-center justify-center h-52 text-center px-4 py-6 text-gray-500 cursor-pointer transition">
              ...
              />
              <span className="text-3xl font-bold text-gray-400">+</span>
              <p className="mt-1 font-medium text-gray-600">Add product images</p>
        ...
        <div className="flex gap-4">
          <button onClick={handlesubmit} className="flex items-center gap-2 px-6 py-3 bg-emerald-500 text-white rounded-lg shadow hover:bg-emerald-600 transition">
            <Save size={18} />
            Save
          </button>
          <button onClick={handlesubmit} className="flex items-center gap-2 px-6 py-3 bg-emerald-500 text-white rounded-lg shadow hover:bg-emerald-600 transition">
