...
const ProductImageUpload = ({
  ...
  console.log("images in product image upload", images);
        ...
        <div className="w-full mt-6 grid grid-cols-3 gap-4 sm:mt-0 sm:ml-4 sm:grid-cols-1 sm:flex sm:flex-wrap">
          {images.map((img, idx) => (
            ...
            >
              ...
              <button
                className="mt-2 flex sm:hidden items-center gap-1 bg-white px-2 py-1 rounded shadow text-red-600 transition-all duration-200 scale-100 cursor-pointer w-full justify-center"
                title="Delete Image"
                onClick={(e) => {
                  e.stopPropagation();
                  handleDeleteClick(img.id);
